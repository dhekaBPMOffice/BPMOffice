...
interface AdminManualSlugPageProps {
  params: Promise<{ slug: string }>;
}

export default async function AdminManualSlugPage({ params }: AdminManualSlugPageProps) {
  const { slug } = await params;
  await requireRole(["admin_master"]);

  const module = ADMIN_MANUAL_MODULES.find((m) => m.id === slug);
  if (!module) {
    notFound();
  }
